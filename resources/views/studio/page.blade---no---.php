          ...
          <div class="card-body">
             <div class="row">
                ...
                <form action="{{ route('editPage') }}" enctype="multipart/form-data" method="post">
                    ...
                    <div class="form-group col-lg-8">
                        <label>{{__('messages.Page Description')}}</label>                        <textarea class="form-control @if(env(\'ALLOW_USER_HTML\') === true) ckeditor @endif" name="pageDescription" rows="3">{{ $page->littlelink_description ?? \'\' }}</textarea>
                    </div>

                    {{-- Campo para Imagem de Topo --}}
                    <div class="form-group col-lg-8">
                      <label class="form-label" for="imagem_topo">{{__("Imagem de Topo (Cabeçalho - 500x200 recomendado)")}}</label>
                      <input type="file" accept="image/jpeg,image/png,image/jpg,image/gif,image/svg" name="imagem_topo" class="form-control" id="imagem_topo">
                      @php
                          $currentUser = Auth::user(); // Get the authenticated user
                          $topoImagePathStudio = public_path("assets/img/topo_images/" . $currentUser->imagem_topo);
                          $topoImageUrlStudio = asset("assets/img/topo_images/" . $currentUser->imagem_topo);
                      @endphp
                      @if($currentUser->imagem_topo && file_exists($topoImagePathStudio))
                          <img class="mt-2" style="max-width:500px; width: 100%; border-radius:5px; margin-bottom: 10px;" src="{{ $topoImageUrlStudio }}">
                          <br>
                          {{-- Optional: Add delete link here if needed --}}
                      @endif
                    </div>
                    ...
                    <div class="form-group col-lg-8">
                        <div class="form-check form-switch">
                            <input class="form-check-input switch toggle-btn" type="checkbox" id="topo_status" name="topo_status" value="1" {{ $currentUser->topo_status ? "checked" : "" }}>
                    ...
                    @if(auth()->user()->role == \'admin\' || auth()->user()->role == \'vip\')                  <div class="form-group col-lg-8">
