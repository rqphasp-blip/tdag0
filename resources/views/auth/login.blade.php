...
@include('layouts.lang')
    ...
    <x-auth-card>
        ...
        <div style="max-width:480px;max-height:550px;" class="container mt-5 w-100">
          <div class="card p-5">
              <a href="{{ url('') }}" class="d-flex align-items-center mb-3">
                <!--Logo start-->
                <div class="logo-main"><center>
                    @if(file_exists(base_path("assets/linkstack/images/").findFile('avatar')))
                    <div class="logo-normal">
                      <img class="img logo" src="{{ asset('assets/linkstack/images/'.findFile('avatar')) }}" style="width:auto;height:40px;">
                  </div>
                  <div class="logo-mini">
                    <img class="img logo" src="{{ asset('assets/linkstack/images/'.findFile('avatar')) }}" style="width:auto;height:40px;">
                  </div>
                    @else
                    <div class="logo-normal">
                      <img class="img logo" type="image/svg+xml" src="{{ asset('assets/linkstack/images/logo.svg') }}" width="30px" height="40px">
                  </div>
                  <div class="logo-mini">
                    <img class="img logo" type="image/svg+xml" src="{{ asset('assets/linkstack/images/logo.svg') }}" width="30px" height="40px">
                  </div>
              ...
              <form method="POST" action="{{ route('login') }}">
                ...
                <p class="text-center my-3"><small>{{__('messages.or sign in with other accounts?')}}</small></p>
                <div class="d-flex justify-content-center">
                ...
                <p class="mt-3 text-center"><center>
                  <h5>{{__('messages.Don’t have an account?')}}</h5> <a href="{{ route('register') }}" class="text-underline">{{__('messages.Click here to sign up')}}.</a></center>
                </p>
                @endif
              </form>
            </div>
