                      ...
                      <section class="text-gray-400">
                        ...
                        <form action="{{ route('editUser', $user->id) }}" enctype="multipart/form-data" method="post">
                          @csrf
                            ...
                            {{-- Campo para Imagem de Topo --}}
                            <div class="form-group col-lg-8">
                              <label>{{__("Imagem de Topo (Cabeçalho - 500x200 recomendado)")}}</label>
                              <div class="mb-3">
                                <input type="file" class="form-control form-control-lg" name="imagem_topo">
                              </div>
                            </div>
                            <div class="form-group col-lg-8">
                                @php
                                    $topoImagePath = public_path("assets/img/topo_images/" . $user->imagem_topo);
                                    $topoImageUrl = asset("assets/img/topo_images/" . $user->imagem_topo);
                                @endphp
                                @if($user->imagem_topo && file_exists($topoImagePath))
                                    <img style="max-width:500px; width: 100%; border-radius:5px; margin-bottom: 10px;" src="{{ $topoImageUrl }}">
                                    <br>
                                    {{-- Adicionar link para deletar imagem de topo se necessário --}}
                                    {{-- <a title="Remover Imagem de Topo" class="hvr-grow p-1 text-danger" style="padding-left:5px;" href="?deleteTopo"><i class="bi bi-trash-fill"></i> {{__("messages.Delete")}}</a> --}}
                                @else
                                    <p><i>{{__("Nenhuma imagem de topo selecionada")}}</i></p>
                                @endif
                                {{-- Adicionar lógica para deletar imagem de topo se o link for clicado --}}
                                {{-- @if(isset($_GET["deleteTopo"]) && $user->imagem_topo && file_exists($topoImagePath)) --}}
                                {{--     @php --}}
                                {{--         File::delete($topoImagePath); --}}
                                {{--         DB::table("users")->where("id", $user->id)->update(["imagem_topo" => null, "data_topo" => null]); --}}
                                {{--         header("Location: ".url()->current()); die(); --}}
                                {{--     @endphp --}}
                                {{-- @endif --}}
                            </div>

                            {{-- Campo para Status da Imagem de Topo --}}
                            <div class="form-group col-lg-8">
                                <div class="form-check form-switch">
                                    <input class="form-check-input" type="checkbox" id="topo_status" name="topo_status" value="1" {{ $user->topo_status ? "checked" : "" }}>
                                    <label class="form-check-label" for="topo_status">{{__("Exibir Imagem de Topo no Perfil?")}}</label>
                                </div>
                            </div>
                            {{-- Fim Campo para Status da Imagem de Topo --}}

                            {{-- Fim Campo para Imagem de Topo --}}
