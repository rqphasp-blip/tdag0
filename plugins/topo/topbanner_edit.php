 ...
 * sem depender de namespaces, ServiceProviders ou includes complexos.
 * 
 * Instruções:
...
if (isset($_POST['banner_upload_action']) && $_POST['banner_upload_action'] == 'upload') {
    ...
    if (!isset($_FILES['banner_image']) || $_FILES['banner_image']['error'] != 0) {
        ...
        if (empty($errors)) {
            ...
            $upload_dir = storage_path('app/public/profile_banners');
            ...
            $path = 'profile_banners/' . $filename;
            
            // Move o arquivo para o storage
            if (move_uploaded_file($file['tmp_name'], storage_path('app/public/' . $path))) {
                ...
                $errors[] = "Erro ao fazer upload do arquivo. Verifique as permissões.";
            }
        }
    }
}
...
<div>
    ...
    <?php if ($user->profile_banner_path): ?>
        ...
        <div class="mt-2">
            <img src="<?php echo e(Storage::url($user->profile_banner_path)); ?>" 
