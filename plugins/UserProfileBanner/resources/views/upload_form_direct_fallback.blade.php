<?php
// Arquivo alternativo para inclusão direta
// Coloque este arquivo em resources/views/userprofilebanner/upload_form_direct_fallback.blade.php

// Este arquivo é uma alternativa para quando o namespace não funcionar
?>
<div>
    @if (session("success"))
    ...
    <form method="POST" action="{{ route('profile.banner.upload.store') }}" enctype="multipart/form-data">
        @csrf
    ...
    @if (Auth::user()->profile_banner_path)
        ...
        <form method="POST" action="{{ route('profile.banner.remove') }}" class="mt-4">
            @csrf
