                ...
                <div class="card-body">
                    ...
                    <form method="POST" action="{{ route("banner.store") }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group mb-3">
                            <label for="banner_image">{{ __("Nova Imagem do Banner (JPG, PNG, GIF, WEBP - Máx 2MB)") }}</label>
                        ...
                        <div class="form-group mb-0">
                            <button type="submit" class="btn btn-primary">
                                {{ __("Enviar Novo Banner") }}
                            </button>
                        </div>
                    </form>

                    {{-- Banner Atual e Botão de Remover --}}
                    @if ($user->profile_banner_path)
                        ...
                        <form method="POST" action="{{ route("banner.destroy") }}" class="mt-3">
                            @csrf
                            @method("DELETE")
                            <button type="submit" class="btn btn-danger">
                                {{ __("Remover Banner Atual") }}
                            </button>
                        </form>
                    @else
                        <hr class="my-4">
