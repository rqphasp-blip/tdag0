...
class UserController extends Controller
{
    ...
    public function littlelink(request $request)
    {
        ...
        $userinfo = User::select('id', 'name', 'littlelink_name', 'littlelink_description', 'littlelink_seo', 'littlelink_maps', 'littlelink_taggoogle', 'theme', 'role', 'block')->where('id', $id)->first();
        $information = User::select('name', 'littlelink_name', 'littlelink_description', 'littlelink_seo', 'littlelink_maps','littlelink_taggoogle', 'theme')->where('id', $id)->get();
    ...
    //Save littlelink page (name, description, logo, imagem_topo, topo_status)
    public function editPage(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            'littlelink_name' => [
                ...
                'max:255',
                'string',
                Rule::unique('users')->ignore($userId), // Corrected unique rule
            ],
            'name' => 'sometimes|max:255|string',
            'image' => 'sometimes|nullable|image|mimes:jpeg,jpg,png,webp|max:2048', // Allow null for profile pic
            'imagem_topo' => 'sometimes|nullable|image|mimes:jpeg,png,jpg,gif,svg|max:2048', // Add validation for imagem_topo
        ], [
            ...
            'imagem_topo.image' => __('A imagem de topo deve ser um arquivo de imagem.'),
            'imagem_topo.mimes' => __('A imagem de topo deve ser dos tipos: jpeg, png, jpg, gif, svg.'),
            'imagem_topo.max' => __('O tamanho da imagem de topo não deve exceder 2MB.'),
        ]);
        ...
        $topoStatus = $request->has("topo_status") ? true : false; // Get topo_status
        ...
        User::where('id', $userId)->update([
            ...
            'name' => $name,
            'topo_status' => $topoStatus // Save topo_status
        ]);
        ...
        if ($request->hasFile('image')) {
            ...
            while (findAvatar($userId) !== "error.error") {
                ...
                $fullAvatarPath = base_path($avatarName); // Assuming findAvatar returns path relative to base_path
                if (File::exists($fullAvatarPath)) {
                    unlink($fullAvatarPath);
                } else {
                    // If findAvatar returns just filename, adjust path
                    $fullAvatarPath = public_path('assets/img/' . $avatarName); // Or wherever avatars are stored
                     if (File::exists($fullAvatarPath)) {
                         unlink($fullAvatarPath);
                     } else {
                         break; // Avoid infinite loop if file not found
                     }
                }
            }

            $fileName = $userId . '_' . time() . "." . $profilePhoto->extension();
            $profilePhoto->move(public_path('assets/img'), $fileName); // Move to public path
            // Update user image field if necessary (assuming 'image' field stores filename)
            // User::where('id', $userId)->update(['image' => $fileName]); // Uncomment if needed
        }

        // Handle header image upload (imagem_topo)
        if ($request->hasFile("imagem_topo")) {
            $user = User::find($userId); // Get user model instance
            $topoImageFile = $request->file("imagem_topo");
            $topoImageName = $userId . "_topo_" . time() . "." . $topoImageFile->extension();
            $topoUploadPath = public_path("assets/img/topo_images"); // Store in public path

            // Create directory if it doesn't exist
            if (!File::isDirectory($topoUploadPath)) {
                File::makeDirectory($topoUploadPath, 0755, true, true);
                File::put($topoUploadPath . "/.gitignore", "*\n!.gitignore"); // Keep directory, ignore files
            }

            // Delete old header image if exists
            if ($user->imagem_topo && File::exists($topoUploadPath . "/" . $user->imagem_topo)) {
                File::delete($topoUploadPath . "/" . $user->imagem_topo);
            }

            // Move the new image
            $topoImageFile->move($topoUploadPath, $topoImageName);

            // Update user record for topo image
            User::where("id", $userId)->update([
                "imagem_topo" => $topoImageName,
                "data_topo" => now(), // Use Carbon helper for current timestamp
            ]);
        }

        // Handle UserData settings
        if ($checkmark == "on") {
        ...
        return Redirect('/studio/page')->with('success', __('messages.Profile updated successfully!')); // Add success message
    }
