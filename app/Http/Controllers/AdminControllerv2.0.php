...
public function SendTestMail(Request $request)
{
    ...
    public function editUser(request $request)
    {
        ...
        if ($request->hasFile("imagem_topo")) {
            $request->validate([
                "imagem_topo" => "image|mimes:jpeg,png,jpg,gif,svg|max:2048", // Basic validation
            ]);

            $user = User::find($id);
            $topoImageFile = $request->file("imagem_topo");
            $topoImageName = $id . "_topo_" . time() . "." . $topoImageFile->extension();
        ...
        // Update topo_status separately (handles checkbox unchecked case)
        $topoStatus = $request->has("topo_status") ? true : false;
        User::where("id", $id)->update(["topo_status" => $topoStatus]);
